import xss from 'xss';
import { STATUS_SUCCESS, XSS_WHITE_LIST } from '~/constants';
...
export default {
  ...
  getCompanyCover (cover) {
    return cover || 'https://www.fpt.com.vn/Images/images/Test-team2.jpg';
  },
  ...
  getFullAddress (address) {
    ...
    let strAddress = address.detail;
    if (address.ward) {
      strAddress += `, ${address.ward.value}`;
    }
    if (address.district) {
      strAddress += `, ${address.district.value}`;
    }
    if (address.province) {
      strAddress += `, ${address.province.value}`;
    }
